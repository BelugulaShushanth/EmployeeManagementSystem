...
window.onload = (e) => {
    ...
    if (!(isLoggedIn == "true")) {
        ...
        employeeImage.src = backEndUrl+"/employee/getEmployeeImageById?employeeId=" + employeeId;
            ...
            .then(function (data) {
                ...
                if (res.length == 0) {
                    ...
                    for (let i = 0; i < res.length; i++) { //iterating throw all the employee objects obtained from API.
                        ...
                        var taskstatus = res[i].status;
                        let changedStatus = "";
                        if (taskstatus.toUpperCase() == "TO DO") {
                            changedStatus = "IN PROGRESS";
                            a1.innerHTML = "Change to IN PROGRESS";
                        }
                        else if (taskstatus.toUpperCase() == "IN PROGRESS") {
                            changedStatus = "TO DO";
                            a1.innerHTML = "Change to TO DO";
                        }
                        var post_url = "";
                        a1.href = post_url;
                        li1.appendChild(a1);
                        ...
                        //Hitting the update api to update the task status by id
                        a1.addEventListener('click', (e) => {
                            e.preventDefault();
                            fetch(backEndUrl+"/task/updateTaskStatusById", {
                                method: 'POST',
                                body: JSON.stringify(
                                    {
                                        "taskId": res[i].taskId,
                            ...
                            .then((response) => {
                                ...
                                .then((data) => {
                                    changeTaskStatus(res[i].taskId, changedStatus);
                                    changedStatus = "";
                        ...
                        var expectedEndDate = document.getElementById('expectedEndDate');
                        ...
                        var closureCriteria = document.getElementById('closureCriteria').value;
                        var comments = document.getElementById('comments').value;
                        a2.addEventListener('click', (e) => {
                            taskTitle.value = res[i].taskTitle;
                            expectedStartDate.value = res[i].expectedStartDate;
                            expectedEndDate.value = res[i].expectedEndDate;
                            description.value = res[i].description;
                            var doneTaskForm = document.getElementById('doneTaskForm');
                            var doneTaskbtn = document.getElementById('doneTaskbtn');
                            ...
                            var updateStatus;
                            doneTaskForm.addEventListener('submit', (e) => {
                                e.preventDefault();
                                containerid.classList.add('hide');
                                loadingGifId.classList.add('display');
                                fetch(backEndUrl+"/task/updateTaskById", {
                                    method: 'PUT',
                                    body: JSON.stringify(
                                        {   
                                            "taskId" : res[i].taskId,
                                            "taskTitle": res[i].taskTitle,
                                            "description": res[i].description,
                                            "status": "DONE",
                                            "expectedStartDate": res[i].expectedStartDate,
                                            "expectedEndDate": res[i].expectedEndDate,
                                ...
                                .then((data) => {
                                        if (updateStatus == 500) {
                                            ...
                                            servererror.innerHTML = "Unable to change task status Server error please try again!"; // To show the custom error msg
                                        }
                                        else {
                                            doneTaskForm.reset();
                                            containerid.classList.remove('hide');
                                            ...
                                            alertStatus.innerHTML = "Task "+ res[i].taskTitle +"Status changed to DONE";
                        ...
                        if (taskstatus.toUpperCase() != "DONE") {
                            ...
                            span.innerHTML = "Changing the status from done is not allowed";
                            dropdown.appendChild(span);
                            image.classList.add("dontEdit");
                        }
                        col5.append(dropdown);
                ...
                function changeTaskStatus(taskId, status) {
                    ...
                    delStatus.classList.add("show");
                    text.innerHTML = "TaskId:" + taskId + "<br> Status Changed to " + status;
